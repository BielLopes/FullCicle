...
export class OrdersService {
  ...
  async initTransaction(input: InitTransactionDto) {
    //prismaService.$use() - dispara eventos dentro do prisma
    ...
    const walletAssets = await this.prismaService.walletAsset.findUnique({
      where: {
        wallet_id_asset_id: {
    ...
    this.kafkaClient.emit('input', {
      ...
      asset_id: order.asset_id,
      current_shares: walletAssets.shares,
      shares: order.shares,
      price: order.price,
  ...
  async executeTransaction(input: InputExecuteTransactionDto) {
    // Todas as transações precisam ser executadas, ou nenhuma é executada - ACID
    // A atomicidade é garantida pela transação e travamento (versionamento)
    return this.prismaService.$transaction(async (prisma) => {
      ...
      await prisma.order.update({
        ...
        data: {
          ...
          // Atualizar o status da ordem como OPEN ou CLOSED
          status: input.status,
          Transactions: {
            create: {
